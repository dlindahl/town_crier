...
Subscriber.prototype.onSubscribe = function(stack) {
  ...
  this.logger('log', 'Client connected to queue: '+queue.name);
  this.req.connection.once('close', queueUnsubscriber(queue, subscription.consumerTag));
};

Subscriber.prototype.parsePayload = function(message, headers, deliveryInfo) {
  var payload = {},
  ...
  return {
    event : data.type || deliveryInfo.routingKey,
    id    : +new Date(),
    data  : JSON.stringify(data)
  };
};

Subscriber.prototype.onMessage = function(message, headers, deliveryInfo) {
  var payload = this.parsePayload(message, headers, deliveryInfo);
