...
function queueBind(stack) {
  ...
  self.emit('beforeQueueBind', self, fanout, queue, routingKey, client);
  ...
  queue.subscribe()
    .addCallback(function(queueSubscription) {
      ...
      self.emit('queueBind', self, fanout, queue, ctag, routingKey, client);
...
function queueName(amqp, client) {
  var env     = process.env.NODE_ENV,
      appName = client.req.params.app_name;

  return amqp.options.queueName || 'town_crier-'+env+'-'+appName+':'+client.userId+':'+client.id;
}
...
function fanoutName(amqp, exchangeName, routingKey) {
  var env = process.env.NODE_ENV;

  return amqp.options.fanoutName || 'town_crier-'+env+'-'+exchangeName+'-'+routingKey;
}
...
function fanoutDeclare(stack) {
  var deferred = when.defer(),
      name     = fanoutName(this, stack.exchangeName, stack.routingKey),
      opts     = {
        type       : 'fanout',
        durable    : false,
        autoDelete : true
