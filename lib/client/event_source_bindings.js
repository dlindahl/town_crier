...
function onQueueBind(es, fanout, queue, ctag, boundKey) {
  this._queueUnsubscriber = queueUnsubscriber.bind(this, fanout, queue, ctag);
  this._onMessage         = onMessage.bind(this, boundKey);
...
function onMessage(boundKey, message, headers, deliveryInfo, rawMessage) {
  var msg = new Message(boundKey, message, headers, deliveryInfo, rawMessage),
      payload = msg.toJSON();
...
function Binder(es) {
  ...
  this.once('unsubscribe', this.destroy);
  this.once('destroy',     this._onDestroy);
  this.once('invalid',     onInvalid);

  // Client TCP Connection
  this.req.connection.once('end',   this._onConnectionEnd);
  this.req.connection.once('close', this._onConnectionClose);

  // Town Crier EventSource Connection
  es.once('error',       this._onEventSourceError);
  es.once('queueBind', this._onQueueBind);
}
